...
int ritornaVerticeAventeGradoMin(const Grafo& g) {
  ...
  int v;
  int save = 1000; //così scendo per forza
  for(int i=0; i<g.numVertici; ++i){ //scorro i vertici
    int peso = 0;
    for(int j=0; j<g.numVertici; ++j){ //scorro
        if(ritornaPeso(g, i, j) != 0)
            peso++;
    }
    if(peso <= save){
        v = i; //aggiorno il vertice più piccolo
        save = peso;
    }
  }
  return v;
...
/*  Verifica se nel grafo g esiste un cammino di nodi C monotono decrescente, ovvero se � possibile
...
bool camminoNelGrafoDecrescente(const Grafo& g, const int& l, const Cammino C) {
    // INSERIRE QUI IL VOSTRO CODICE
    int primoPeso = ritornaPeso(g, C[0], C[1]); //peso del primo arco
    for(int i=1; i<l-1; i++){ //parto dal secondo arco e mi fermo a l-1 per evitare seg fault
        if(ritornaPeso(g, C[i], C[i+1]) <= primoPeso){ //se va tutto bene
            primoPeso = ritornaPeso(g, C[i], C[i+1]); //aggiorno il valore
        } //e vado avanti nello scorrere il cammino
        else
            return false; //non è decrescente
    } //se non sono stato interrrotto
    return true;
}

/* Conta e restituisce il numero dei sink presenti nel grafo g
 Un sink in un grafo � un vertice che non ha archi uscenti (cioe' un vertice senza outgoing edges)*/
