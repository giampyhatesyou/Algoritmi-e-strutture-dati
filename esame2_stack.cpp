...
char returnFirstParenthesis(string &str) {
 // INSERIRE QUI IL VOSTRO CODICE
    char cpy = EMPTYCHAR; //valore default
        for(int i=0; i<str.size(); ++i){ //scorro tutta la stringa finchè non trovo una parentesi
            if(str[i] == ')' || str[i] == '(')
                cpy =  str[i]; //assegno a cpy il valore della parentesi
            else
                str.erase(0,1);
    }
...
bool parenthesisArebalanced(string &str) {
   // INSERIRE QUI IL VOSTRO CODICE
   int counter = 0; //ogni volta che apro una parentesi aumento di uno, mi aspetto che torni a zero decrementando quando vinee chiusa
   ...
   return (counter==0); //se è uguale a zero vuol dire che ogni parentesi aperta è stata chiusa
}
...
int countParenthesis(stack::Stack& s) {
  // INSERIRE QUI IL VOSTRO CODICE
  int counter = 0;
  if(!stack::isEmpty(s)){ //solo se non è vuoto
    do{
        char cpy = pop(s); //devo a svuotare la pila
        if(cpy == ')' || cpy == '(') //se è una parentesi
            counter++;
    }while(!stack::isEmpty(s));//finchè non svuoto
  }
  return counter;
}
