...
//FUNZIONI AUSILIARIE//
//*******************//
int ricorrenze(const int a[], int x){ //funzione che conta le ricorrenze di un elemento in un array
    int cont=0;
    for(int i=0; i<dim; i++){
        if(a[i]==x)
            cont++;
...
int difference(const int a[], const int b[], int c[]) {
	int size = 0;
	for(int i=0; i<dim; i++){ //scorro a
	    if(ricorrenze(b, a[i]) == 0){ //e controllo non appartenga a b
	        c[size] = a[i]; //inserisco in c
	        size++;
...
int intersection(const int a[], const int b[], int c[]) {
    int size = 0; //contatore per size di c
    for(int i=0; i<dim; i++){ //controllo che l'elemento a[i] sia in entrambi gli insiemi
        if(ricorrenze(a, a[i]) > 0 && ricorrenze(b, a[i]) > 0){
            c[size]=a[i];
            size++;
        }
    }
...
bool equal(const int a[], const int b[]) {
    for(int i=0; i<dim; i++){
        if(ricorrenze(a, a[i]) != ricorrenze(b, a[i]))
            return false;
    }
    return true;
}
