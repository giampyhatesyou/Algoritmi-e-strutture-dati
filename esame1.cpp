...
void print_list(std::ostream& output_stream, basic_list list) {
   // INSERIRE QUI IL VOSTRO CODICE
    if(list != emptyList){
      basic_list aux = list;
      do{
          std::cout << aux->payload.id << ", " << aux->payload.azienda << ", " << aux->payload.subtotale << std::endl;
          aux = aux->next;
      } while(aux != list); //essendo liste circolari saprò di  essere alla fine quando torno al primo elemento
    }
    std::cout << "FINE\n";
} 
...
void tail_insert(basic_list& list, fattura_t new_value) {
  // INSERIRE QUI IL VOSTRO CODICE
  cell* aux = new cell; //creo nuova cella da inserire
  aux->payload = new_value;
  aux->next = list;
  basic_list cur = list;
  do{
    cur = cur->next; //arrivo al primo puntatore next libero (ovvero la fine)
  } while(cur->next != list);
  cur->next = aux;
}
...
float compute_total(basic_list& list) {
  ...
  basic_list cur = list;
  do{
    tot += cur->payload.subtotale;
    cur = cur->next;
  } while(cur != list);
  float iva = (tot/100)*22;
  return tot+iva;
}
