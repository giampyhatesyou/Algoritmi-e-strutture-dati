...
int numeroTotaleOreMeeting(Grafo grafo, int id_persona) {
    ...
    int tot = 0;
    for(int i=0; i<grafo.n_persone; ++i){
        tot+=grafo.meetings[id_persona][i];
    }
    return tot;
}
...
Lista personeCheNonHannoAvutoMeeting(Grafo grafo) {
    ...
    Lista l = lista_vuota;
    for(int i=0; i<grafo.n_persone; ++i){
        if(numeroTotaleOreMeeting(grafo, i) == 0)
            aggiungiElemento(l, i);
    }
    return l;
}

//--FUNZIONI AUSISLIARIE PER LA FUNzIONE 4---
Lista contatti(Grafo grafo, int id){ //funzione che restituisce una lista contenente le persone con cui è stato in contatto id
    Lista l = lista_vuota;
    for(int i=0; i<grafo.n_persone; ++i){
        if(grafo.meetings[i][id] >= 3)
            aggiungiElemento(l, i);
    }
    return l;
}

bool is_in(Lista l, int id){ //funzione ausliaria che restituisce true se id è presente nella lista
    while(l != lista_vuota){
        if(l->id == id)
            return true; //interrompo la funzione
        l = l->prossimo;
    }
    return false; //se non mi sono interrotto allora non è presente
}
...
Lista possibiliLeakInformazioni(Grafo grafo, int id_persona){
    // DA IMPLEMENTARE
    Lista l = lista_vuota;
    aggiungiElemento(l, id_persona); //aggiungo id_persona così non rischio di ripeterlo e posso rimuoverlo facilmente
    for(int i=0; i<grafo.n_persone; ++i){
        if(grafo.meetings[id_persona][i] >= 3){
            if(!is_in(l, i))
                aggiungiElemento(l, i);
            
            Lista aux = contatti(grafo, i); //ottengo lista di tutti i contatti indiretti
            while(aux != lista_vuota){
                if(!is_in(l, aux->id))
                    aggiungiElemento(l, aux->id);
                aux = aux->prossimo; //avanzo
            } 
        }       
    } //rimuovo id_persona dalla lista
    Lista fly = l;
    l = l->prossimo;
    delete fly;
    return l;
